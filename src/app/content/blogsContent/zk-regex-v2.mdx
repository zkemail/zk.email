...
The v1 architecture, while powerful for its intended applications, was based on an approach—directly translating Deterministic Finite Automata (DFAs) into circuits—that involved inherent architectural trade-offs. This design choice was effective for its specific use cases, but presented challenges for supporting a broader set of standard regex syntax, and for the patterns it did support, resulted in circuits whose size and cost scaled with DFA complexity.
...
-   **Circuit Complexity and Size**: Directly encoding a DFA's transition table into arithmetic constraints meant that circuit size and proving cost scaled with the number of states in the DFA.
