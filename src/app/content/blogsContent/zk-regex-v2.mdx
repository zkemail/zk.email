...
# zk-Regex v2: A New Architecture for Verifiable Regex Proofs
...
6.  [Applications and Broader Impact](#6-applications-and-broader-impact)
    -   [On-Chain Data Validation](#on-chain-data-validation)
    -   [Private Information Extraction](#private-information-extraction)
    -   [Privacy-Preserving Content Moderation](#privacy-preserving-content-moderation)
    -   [Verifiable Credentials and Attestations](#verifiable-credentials-and-attestations)
    -   [Decentralized Information Markets](#decentralized-information-markets)
...
However, the v1 architecture, which directly translated Deterministic Finite Automata (DFAs) into circuits, had fundamental limitations. Its design made it difficult or impossible to support a significant portion of the standard regex syntax, particularly features involving non-determinism or dynamic evaluation. For the patterns it did support, the direct-to-circuit translation often resulted in large and inefficient circuits.

This article introduces zk-Regex v2, which uses a new architecture to address these limitations. The v2 model separates the complex task of regex matching from the in-circuit verification, enabling support for a much broader set of regex features with more efficient circuit construction. We will detail the technical innovations in v2, which enable more powerful and flexible ZK-based text processing, setting the stage for applications beyond regex, such as verifiable JSON parsing.
...
Capture groups were handled by tracking when the DFA traversal entered and exited the states corresponding to a part-DFA marked `is_public: true`. Input bytes processed within these state segments were conditionally outputted.

#### 4. Core Challenges Faced by v1:

This direct DFA-in-circuit approach had several fundamental problems:

-   **Circuit Complexity and Size**: Directly encoding a DFA's transition table into arithmetic constraints resulted in large circuits with high proving costs, especially for regexes that produce large DFAs.

-   **Limited Regex Feature Support**: The architecture struggled with regex features that generate significant non-determinism. Constructs like complex alternations (`|`) or unbounded, non-greedy repetitions (`*?`, `+?`) often cause an explosion in the number of states when converted to a DFA, making the corresponding circuit impractically large or impossible to generate.

-   **Fragile Capture Group Composition**: The strategy of manually joining part-DFAs was a brittle workaround. The resulting automaton was not guaranteed to be equivalent to a DFA generated from the complete regex, potentially leading to subtle semantic errors.

-   **State Explosion**: Common regex constructs like `.*` or patterns with wide character ranges and unbounded repetitions often cause an explosion in the number of DFA states, making the corresponding circuits impractically large.

These challenges motivated the new architecture in zk-Regex v2.

### The v2 Vision: Separating Matching from Verification

The limitations of v1 led to a new design centered on Non-deterministic Finite Automata (NFAs) and a separation of concerns: perform complex regex matching off-circuit, and use the circuit only for efficient verification.
...
-   **Robust NFA Construction**: v2 uses Rust's mature `regex_automata` library, which provides optimized and well-tested NFA representations directly from regex patterns.
-   **Native Capture Group Support**: The NFAs produced by `regex_automata` use special epsilon transitions (state transitions that do not consume input characters) to natively mark the entry and exit points of capture groups. This is a more robust solution than v1's part-DFA stitching and allows a single, cohesive NFA to represent the entire pattern with its capture groups.
...
While the general concept of removing epsilon transitions is standard in automata theory, the implementation in zk-Regex v2 is specifically designed to maintain the integrity of capture group boundaries. The key assumption is that any meaningful capture group to be extracted must consume at least one byte.
...
This set of transitions becomes the immutable ruleset embedded within the ZK circuit. The method for checking against this ruleset depends on the features of the target circuit DSL.
...
Noir's sparse array uses a Random Linear Combination (RLC) to generate a unique key for each transition, preventing collisions. For a transition `(current_state, byte, next_state)`, the key is calculated as:

```
key = current_state + byte * 257 + next_state * 257^2
```

where `257` is chosen to be larger than the maximum byte value (255), ensuring each component of the tuple occupies a unique position in the resulting value.
...
-   **Decoupled Complexity**: Circuit size is no longer directly tied to the complexity of the regex pattern. Instead, it is primarily influenced by the maximum length of the _match_ and the number of capture groups. A complex regex that produces a short match can result in a relatively small circuit.
...
v1 handled capture groups by manually splitting a regex into parts. This process was cumbersome and could lead to semantic inconsistencies.

zk-Regex v2 handles capture groups more robustly:

-   Capture groups are a native feature of the initial NFA generated by `regex_automata`.
...
-   **Feature Completeness**: While the architecture theoretically supports any regex compilable by `regex_automata`, extensive testing across all syntax edge cases is ongoing.
...
_(A detailed technical paper with a formal specification of the epsilon elimination algorithm and performance benchmarks is forthcoming.)_
